 ...
 *
 * Integration tests for ShieldedCheckpointClient against deployed v0.8 testnet.
 *
 * Tests (in order):
 *   1. encryptAndUpdate → read → readAndDecrypt: snapshot round-trip
...
describe("ShieldedCheckpointClient — integration", () => {
  ...
  beforeAll(async () => {
    ...
    aliceJub = await deriveMemoKeypair(alice.address, "checkpoint-test:alice:v1");
  ...
  it("should write encrypted snapshot and read it back decrypted", async () => {
    if (SKIP) return;

    // Write checkpoint at cursor CURSOR_INITIAL
    ...
    // Read raw checkpoint (owner-only)
    const raw = await checkpointClient.read(alice);
  ...
  it("exists() should return true for Alice after updates", async () => {
    if (SKIP) return;

    const doesExist = await checkpointClient.exists(alice.address);
    expect(doesExist).toBe(true);
    console.log(`[Checkpoint] exists(alice)=${doesExist}`);
  }, 15_000);

  // ---------------------------------------------------------------------------
  // Test 6 — address constant matches expected deployed address
  // ---------------------------------------------------------------------------

  it("contract address should match SHIELDED_CHECKPOINT_ADDRESS", () => {
    if (SKIP) return;
