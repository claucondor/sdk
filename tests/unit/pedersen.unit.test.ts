...
describe("computeCommitment", () => {
  ...
  it("throws if value >= 2^128 (v0.8 128-bit cap)", async () => {
    // v0.7 used 2^64; v0.8 aggregate circuit uses Num2Bits(128) so cap is 2^128
    await expect(computeCommitment(1n << 128n, 0n)).rejects.toThrow(
      "value must be in [0, 2^128)"
    );
  });

  it("throws if blinding >= SUBORDER (2^252 bits, v0.8 aggregate circuit)", async () => {
    // SUBORDER ≈ 2^251.7; use 2^252 as a value guaranteed to be >= SUBORDER
    const SUBORDER = 2736030358979909402780800718157159386076813972158567259200215660948447373041n;
    await expect(computeCommitment(1n, SUBORDER)).rejects.toThrow(RangeError);
  });
  ...
  it("max valid blinding (2^128 - 1) is accepted (well below SUBORDER)", async () => {
